...
public class ReplyController {
	...
	public ResponseEntity<String> create(@RequestBody ReplyVO vo) { // 들어온 Json데이터를 reply객체로 변환
		...
		return insertCount == 1 ? new ResponseEntity<String>("success", HttpStatus.OK) : new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);	
	}
	
	@GetMapping(value = "/pages/{boardId}/{page}",produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
	public ResponseEntity<List<ReplyVO>> getList(@PathVariable("page") int page, @PathVariable("boardId") Long boardId){
		log.info("특정 게시물의 댓글 목록 확인");
		Criteria cri = new Criteria();
		cri.setAmount(10);
		return new ResponseEntity<List<ReplyVO>>(replyService.getList(cri, boardId), HttpStatus.OK);
	}
	...
	@PostMapping(value = "/{replyId}", produces = {MediaType.TEXT_PLAIN_VALUE})
	public ResponseEntity<String> remove(@PathVariable("replyId") Long replyId){
		log.info("댓글 삭제");
		int removeCount = replyService.remove(replyId);
		
		return removeCount == 1 ? new ResponseEntity<String>("success", HttpStatus.OK) : new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR); 
	}
	...
	public ResponseEntity<String> modify(@PathVariable("replyId") Long replyId,@RequestBody ReplyVO vo){
		...
		return replyService.modify(vo) == 1 ? new ResponseEntity<String>("success", HttpStatus.OK) : new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
}
